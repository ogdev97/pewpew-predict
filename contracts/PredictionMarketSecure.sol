...
/**
 * @title PredictionMarketSecure
 * @dev Secure prediction market with reentrancy protection and safety mechanisms
 */
contract PredictionMarketSecure is ReentrancyGuard, Pausable {
    ...
    uint256 public constant GRACE_PERIOD = 7 days; // Grace period after market end
    
    mapping(address => bool) public admins;
    
    enum Outcome { NONE, YES, NO }
    enum MarketStatus { ACTIVE, ENDED, CANCELLED, EXPIRED }
    ...
    // Track user addresses per market for emergency refunds
    mapping(uint256 => address[]) private marketUsers;
    mapping(uint256 => mapping(address => bool)) private isUserInMarket;
    ...
    event EmergencyRefund(uint256 indexed marketId, address indexed user, uint256 amount);
    ...
    function createMarket(
        ...
        markets[newMarketId] = Market({
            ...
            exists: true,
            participantCountYes: 0,
            participantCountNo: 0
        });
    ...
    function predict(
        uint256 _marketId,
        uint256 _outcome
    ) external payable marketExists(_marketId) marketActive(_marketId) whenNotPaused nonReentrant {
        ...
        if (outcome == Outcome.YES) {
            ...
            market.participantCountYes++;
        } else {
            market.totalNoAmount += msg.value;
            market.participantCountNo++;
        }
        ...
        // Track user for potential refunds
        if (!isUserInMarket[_marketId][msg.sender]) {
            marketUsers[_marketId].push(msg.sender);
            isUserInMarket[_marketId][msg.sender] = true;
        }
     ...
     */
    function claimReward(uint256 _marketId) external marketExists(_marketId) marketEnded(_marketId) nonReentrant {
    ...
    /**
     * @dev Emergency refund if market expired without resolution
     */
    function emergencyRefund(uint256 _marketId) external marketExists(_marketId) nonReentrant {
        Market storage market = markets[_marketId];
        
        // Check if market expired (endTime + grace period passed without resolution)
        require(
            market.status == MarketStatus.ACTIVE && 
            block.timestamp > market.endTimestamp + GRACE_PERIOD,
            "Market not expired"
        );
        ...
        uint256 refundAmount = prediction.amount;
        
        // Mark as claimed and set market as expired
        prediction.claimed = true;
        if (market.status == MarketStatus.ACTIVE) {
            market.status = MarketStatus.EXPIRED;
            emit MarketExpired(_marketId);
        }
        ...
        emit EmergencyRefund(_marketId, msg.sender, refundAmount);
    }
    ...
    /**
     * @dev Check if market is expired and eligible for refund
     */
    function isMarketExpired(uint256 _marketId) external view returns (bool) {
        Market storage market = markets[_marketId];
    ...
    /**
     * @dev Get participant counts
     */
    function getParticipantCounts(uint256 _marketId) external view returns (uint256 yesCount, uint256 noCount) {
        Market storage market = markets[_marketId];
        return (market.participantCountYes, market.participantCountNo);
    }
